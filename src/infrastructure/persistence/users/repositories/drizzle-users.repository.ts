...
export class DrizzleUsersRepository implements UsersRepository {
  ...
  async create(user: User): Promise<User> {
    ...
    const [createdUser] = await this.db.insert(usersTable).values(userEntity).returning()
    return DrizzleUserMapper.toDomain(createdUser)
  }

  async update(user: User): Promise<User> {
    const userEntity = DrizzleUserMapper.toPersistence(user)
    const [updatedUser] = await this.db.update(usersTable).set(userEntity).where(eq(usersTable.id, user.id.value)).returning()
